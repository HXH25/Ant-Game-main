...
- **在结算阶段**，本回合每使用防御塔或超级武器杀死一只对方蚂蚁，按照对方蚂蚁的等级，等级1的蚂蚁给予6金币，等级2的蚂蚁给予10金币，等级3的蚂蚁给予14金币。本回合每有一只蚂蚁到达敌方基地，给予10金币。因寿命死亡的蚂蚁不对金币产生影响。
...
- **防御塔数据列表**
  ...
  | 1      | Heavy              | 20   | 2    | 1    | 单体攻击 | 0 |
  | 11     | Heavy+             | 30   | 2    | 2    | 单体攻击 | 1 |
  | 12     | Ice                | 10   | 2    | 1    | 单体攻击；命中的蚂蚁若非免控，则冻结1回合，解冻后转为**随机型** | 1 |
  | 13     | Bewitch            | 10   | 2    | 2    | 单体攻击；命中的蚂蚁若非免控，则转为**蛊惑型**。若其在己方半场，目标为己方基地；若已在敌方半场，则以过该蚂蚁、平行于分界线的线为界，取朝己方基地一侧区域内的随机合法位置为目标 | 1 |
  | 2      | Quick              | 5    | 1    | 1    | 单体攻击 | 0 |
  | 21     | Quick+             | 5    | 0.5  | 1    | 单体攻击 | 2 |
  | 22     | Double             | 5    | 2    | 3    | 最多攻击优先级前二的目标 | 2 |
  | 23     | Sniper             | 10   | 2    | 4    | 单体攻击 | 2 |
  | 3      | Mortar             | 10   | 4    | 2    | 以目标点为中心、范围2的AOE | 0 |
  | 31     | Mortar+            | 15   | 4    | 2    | 以目标点为中心、范围2的AOE | 3 |
  | 32     | Pulse              | 5   | 4    | 2    | 同时攻击自身范围内所有敌方蚂蚁，并将命中的蚂蚁转为**随机型** | 3 |
  | 33     | Missile            | 20   | 6    | 3    | 以目标点为中心、范围3的AOE | 3 |
  | 4      | Producer           | 0    | 8    | 0    | 不主动攻击；周期性在相邻路径格产出普通蚂蚁 | 0 |
  | 41     | Producer+       | 0    | 6    | 0    | 不主动攻击；更快地产出普通蚂蚁 | 4 |
  | 42     | Siege      | 0    | 8    | 0    | 不主动攻击；正常产出普通蚂蚁，并以25%概率额外产出1只**战斗蚂蚁** | 4 |
  | 43     | Medic      | 0    | 8    | 前方不限 | 不主动攻击；正常产出普通蚂蚁；每4回合为前线两行内的己方蚂蚁治疗至满血并附加1层**紧急回避**，优先战斗蚂蚁，再选当前血量最低者 | 4 |
...
| ID   | 名称                       | 花费 | 冷却 | 效果                                                         |
| ---- | -------------------------- | ---- | ---- | ------------------------------------------------------------ |
| 1    | 闪电风暴 Lightning Storm   | 150  | 100  | 令指定位置**范围为3**的区域内进入“闪电风暴”状态，**持续20回合**。每回合对范围内的**所有**敌方“工蚁”造成100伤害。每回合结束时，区域中心会在**当前位置**与相邻六个方向对应的**合法位置**中随机选择一格，作为下一回合的位置（若部分相邻格超出地图，则不参与选择）。 |
| 2    | EMP轰炸 EMP Blaster        | 150  | 100  | 令指定位置**范围为3**的区域内陷入**“电磁脉冲干扰”**状态，**持续20回合**。敌方无法在有“电磁脉冲干扰”的区域内建造、升级、降级防御塔（超级武器不受影响）。在“电磁脉冲干扰”区域内的敌方防御塔无法攻击，内置CD不变。每回合结束时，区域中心会在**当前位置**与相邻六个方向对应的**合法位置**中随机选择一格，作为下一回合的位置（规则同上）。 |
| 3    | 引力护盾 Deflectors        | 100  | 50   | 令指定位置**范围为3**的区域内进入**“引力护盾”**状态，**持续10回合**。在“引力护盾”区域内的己方蚂蚁免疫单次小于自身最大生命值50%的伤害。对于大于等于自身最大生命值50%的伤害不影响。当蚂蚁**离开**该状态，或该状态**结束**时，若其尚未是免控型，则转为**免控型**，并从进入免控型起独立计时，至第5回合结束时退化为**默认型**。 |
| 4    | 紧急回避 Emergency Evasion | 100  | 50   | **立刻**将指定位置**范围为3**的区域内所有我方“工蚁”的“紧急回避”层数补至**至少2层**；并且在**本回合攻击结算前**，仍位于该区域内的我方“工蚁”会再次被补至**至少2层**。每层“紧急回避”可以抵消一次防御塔造成的伤害（优先于“引力护盾”的效果进行结算）。不会过期。当“紧急回避”层数耗尽时（即最后一次抵消伤害后），该蚂蚁转变为**免控型**（如果尚未是免控型），并从进入免控型起独立计时，至第5回合结束时退化为**默认型**。 |

### 6. 基地

- 每方基地在游戏开始时有50血量，每当一只敌方“工蚁”移动到基地，则会减少1血量。当有一方基地血量不大于0时游戏立即结束。
- 基地可以进行如下升级：
- **优化生产流水线**：基地会按下表对应的生产周期自动建造一只“工蚁”。初始为1级；当周期为 `3.5` 时，会按 `0, 4, 7, 11, 14, ...` 这样的离散节奏产兵。
...
蚂蚁现在同时拥有**兵种(kind)**与**行为(behavior)**两个维度。行为决定移动方式，兵种决定移动画像与拆塔能力。所有蚂蚁在移动时都遵循以下基本约束：
...
- 免控型蚂蚁免疫冻结、行为转化等控制效果，且不计入每10回合的随机传送选择中。

#### 行为特性

- **默认型**：使用风险感知寻路。每个候选方向会综合考虑“推进敌方基地的收益、信息素、拥塞、预期伤害、控制风险、战斗蚂蚁对塔的吸引度”，再通过 softmax 采样决定方向。当前默认温度参数为 `1.75`。
- **保守型**：与默认型使用同一套评分函数，但不采样，而是直接选择分数最高的方向。进入保守型后的第5回合结束时退化为**默认型**。
- **随机型**：在合法候选方向中均匀随机选择。随机型在进入后的第5回合结束时退化为**默认型**。
- **蛊惑型**：目标不再是敌方基地，而是由 `Bewitch` 指定的内部目标格；其余仍使用风险感知评分，但更偏向朝该目标推进，并允许回头。若提前抵达目标格，或持续5回合结束，则退化为**默认型**。
- **免控型**：移动规则与保守型一致，但免疫控制。进入免控型后的第5回合结束时退化为**默认型**。

#### 风险感知寻路

对默认型、保守型、免控型以及蛊惑型蚂蚁，当前移动评分可概括为：

$$
\mathrm{score} = w_p \cdot progress + w_\tau \cdot pheromone - w_c \cdot crowding - w_d \cdot damage - w_r \cdot control + w_t \cdot towerPull
$$
...
2. 回合结算
   ...
   2. 结算闪电风暴：对所有处于闪电风暴**当前区域**内的敌方蚂蚁造成100伤害。
   ...
   7. 结算信息素并移除已死亡、已到达的蚂蚁；到达的蚂蚁结算金币收益（每只10金）。
