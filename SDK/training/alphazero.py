...
class AlphaZeroSelfPlayTrainer:
    ...
    def train(self, num_batches: int | None = None) -> tuple[list[dict[str, float]], list[EpisodeSummary]]:
        ...
        print(f"开始AlphaZero训练，共{updates}批次...")
        
        for batch_index in range(updates):
            print(f"\n=== 批次 {batch_index + 1}/{updates} ===")
            episode_batches = []
            episode_summaries = []
            
            for episode_offset in range(self.config.episodes):
                print(f"  收集游戏 {episode_offset + 1}/{self.config.episodes}...")
            ...
            # 打印当前进度和指标
            print(f"  批次完成！")
            print(f"  胜率: {metrics.get('eval_win_rate', 0):.4f}")
            print(f"  平局率: {metrics.get('eval_draw_rate', 0):.4f}")
